...
# instead of loading the weights locally. This file targets the gemma-4-12B-it
# server (port 8003); the generic g24_rollback.py targets Qwen3-8B on 8002.
...
# gemma-4-12B-it is an instruction/chat model: fed the analysis few-shot as a
# raw *completion* it falls into a repetition loop echoing the prompt header
# ("##Your analysis of the current trajectory" over and over) and never emits
# a parseable "** Error Location **" block. Routing the analysis step through
# the *chat* endpoint with this system message fixes it -- gemma then produces
# a coherent analysis ending in the required Conclusion format.
# Brevity is load-bearing: left to ramble, gemma explores candidate solutions
# and blows the token budget before ever reaching the "### Conclusion" block,
# leaving nothing for split_analysis() to parse. Capping the analysis and
# forbidding solution search makes it land the required format every time.
ANALYSIS_SYSTEM_PROMPT = (
    "You analyze Game-of-24 trajectories to find the earliest reasoning error. "
    "Follow the format of the example cases EXACTLY. "
    "Keep the '### Analysis' section to at most 3 short sentences. "
    "Do NOT search for, propose, evaluate, or list any arithmetic expressions "
    "or alternative solutions anywhere in your answer. "
    "Immediately output a '### Conclusion' section with EXACTLY two lines and "
    "nothing after:\n"
    "** Error Location **: <Act N, or None if there is no error>\n"
    "** Explanation **: <ONE sentence, under 25 words, naming the step and why "
    "it fails; no expressions>"
)

# gemma's analysis is generated over the whole chain-of-thought via chat, so the
# mean per-token probability runs lower than Qwen's short raw completion. The
# rollback trust gate (prob_threshold) is therefore looser here by default;
# override with --prob_threshold if needed.
prob_threshold = 0.80
...
class VLLM_llm:
    ...
    def _generate(self, prompt, stop=None, max_new_tokens=100, gen_logits=False):
        ...
        # gen_logits marks the rollback *analysis* call. gemma-it degenerates on
        # this as a raw completion, so use the chat endpoint (with a format-
        # pinning system prompt) and pull per-token logprobs from the chat
        # response. Action generation (gen_logits=False) still uses the raw
        # completion path below, which gemma handles fine (matches g24_react.py).
        if gen_logits:
            ...
            response['text'] = (choice.message.content or "").strip()

            # average_prob / logsumexp_prob over the generated analysis tokens,
            # same features as the completion path (chat logprobs live under
