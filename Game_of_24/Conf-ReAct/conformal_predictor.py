...
class ConformalPredictor:
    ...
    def calibrate_from_records(                 # given the val_records, it computes the non-confirmty scores of all optimal actions, stores it and its corresponding state embeddings.
        ...
        for rec in val_records:
            ...
            # SET-COVERAGE (game24, patch #2): a state is "covered" if the
            # prediction set contains ANY correct action, so the state's
            # nonconformity is the score of the BEST (lowest) correct action.
            correct_idxs = rec.get('correct_idxs', [rec['optimal_idx']])

            nc = self.score_model.compute_nonconformity_scores(
                state_raw, action_embs, softmax_bins
            )  # [K]   # nonconformity score for every admissible action

            scores.append(min(nc[i].item() for i in correct_idxs))   # best correct action
