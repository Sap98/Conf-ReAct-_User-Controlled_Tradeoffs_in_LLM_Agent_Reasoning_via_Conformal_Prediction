"""GA-Rollback on WebShop — identical to web_rollback_gpt4.1.py except the
inference model: the Azure OpenAI gpt-4.1 chat endpoint is replaced by the
local vLLM Gemma-4-12B-it completions endpoint (with logprobs for the
analysis-quality gate), restoring the original GA-Rollback Api_llm. The
`website` env module and the prompt files are loaded from the original
GA-Rollback tree. The Local_llm (open-source HF) path is kept; its torch /
transformers imports are lazy so this file runs without torch installed.
...
with open(os.path.join(_GA_DIR, "prompts/webshop/webshop_ssr_actionly_prompt.txt"), 'r') as f:
    BASE_PROMPT = f.read()

with open(os.path.join(_GA_DIR, "prompts/webshop/webshop_ssr_analyze_examples.txt"), 'r') as f:
    ANALYZE_EXAMPLE = f.read()

with open(os.path.join(_GA_DIR, "prompts/webshop/webshop_ssr_repetition_examples.txt"), 'r') as f:
    REPETITION_EXAMPLE = f.read()

MODEL_NAME = "google/gemma-4-12B-it"                        # For vllm use
client = OpenAI(base_url="http://10.5.18.73:8003/v1", api_key="EMPTY")
prob_threshold = 0.93
prob_list = []
logsum_list = []
PLOT_PREFIX = "web_rollback_gemma4_12b_traj_hist"
RESULTS_LOG = "web_rollback_gemma4_12b_traj_dist.txt"
...
class Api_llm:
    ...
    def _generate(self, prompt, stop=None, max_new_tokens=100, gen_logits=False):
        ...
        # Gemma-it is instruction-tuned: in raw-completion mode it echoes the
        # instruction instead of continuing "Action N:", so use the chat
        # endpoint (as the gpt-4.1 variant did). vLLM returns per-token
        # logprobs for chat completions, which the analysis-quality gate needs.
        completion = client.chat.completions.create(
            ...
            logprobs=True if gen_logits else False,
        )
        choice = completion.choices[0]
        generate_text = choice.message.content or ''
        if stop:
            parts = [text for text in generate_text.split(stop) if text]
            generate_text = parts[0] if parts else ''
        response['text'] = generate_text.strip(' ')

        if gen_logits and choice.logprobs and choice.logprobs.content:
            token_logprobs = [tok.logprob for tok in choice.logprobs.content
...
def step_with_search_redirect(env, idx, action):
    # Corner case: search[] is only valid from the search page (page_type 'init').
    # If search[] is proposed while on a results/item page, first revert to the
    # search page via click[Back to Search], then run the search.
    if (action.startswith('search[')
            and env.sessions.get(idx, {}).get('page_type', 'init') != 'init'):
        env.step(idx, 'click[Back to Search]')
    return env.step(idx, action)
...
def rollback(idx: str, env, env_history, llm, e_loc: int, exp: List):
    ...
    for action in act_list:
        _ = step_with_search_redirect(env, idx, action)
...
def webshop_run(idx, env, base_prompt, llm, assist_llm, is_assist, to_print=True, max_rollback_num=6):
    ...
    while True:
        ...
        try:
            res = step_with_search_redirect(env, idx, action)
            observation = res[0]
...
if __name__ == '__main__':
    ...
    parser.add_argument("--out_record_path", type=str, default="rollback_records_gemma4_12b.json", help="the file path for saving rollback records")
