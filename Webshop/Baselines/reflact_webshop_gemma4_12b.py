...
Identical to reflact_webshop_gpt4.1.py except the inference model: the Azure
OpenAI gpt-4.1 chat endpoint is replaced by the local vLLM Gemma-4-12B-it
completions endpoint (same backend as react_webshop_qwen3_8b.py).
...
Changes vs the upstream notebook (and ONLY these):
  ...
  2. llm() now calls local vLLM Gemma-4-12B-it via the OpenAI-compatible
     /completions endpoint (the notebook's text-davinci-002 is retired).
     Greedy (temperature=0), stop=['\\n'], same single-trajectory loop.
...
Run:
  python reflact_webshop_gemma4_12b.py            # default 50 episodes (fixed_0..49)
  python reflact_webshop_gemma4_12b.py 500        # full eval like the notebook
  python reflact_webshop_gemma4_12b.py 50 100     # 50 episodes starting at fixed_100
  python reflact_webshop_gemma4_12b.py 500 0 > reflact_webshop_qwen_25.txt   # capture stdout
"""
...
RESULTS_LOG = "reflact_webshop_gemma4_12b_results.txt"
PLOT_PREFIX = "reflact_webshop_gemma4_12b_traj_hist"


# Gemma-4-12B-it is instruction-tuned: on the raw /completions endpoint it does
# NOT continue "Action:" literally — it degenerates (loops/repeats corrupted
# text). So use the chat endpoint with a system message that pins the output to
# exactly one action, then strip any "Action:"/"Action N:" prefix the model still
# adds. (Same fix web_rollback_gemma4_12b.py already applies.)
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            {"role": "system", "content":
                ...
                "and nothing else. Valid actions: search[...], click[...], reflection[...]. "
