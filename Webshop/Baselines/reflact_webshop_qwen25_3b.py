...
OpenAI gpt-4.1 chat endpoint is replaced by the local vLLM Qwen2.5-3B
...
Changes vs the upstream notebook (and ONLY these):
  ...
  2. llm() now calls local vLLM Qwen2.5-3B via the OpenAI-compatible
...
Run:
  python reflact_webshop_qwen25_3b.py            # default 50 episodes (fixed_0..49)
  python reflact_webshop_qwen25_3b.py 500        # full eval like the notebook
  python reflact_webshop_qwen25_3b.py 50 100     # 50 episodes starting at fixed_100
  python reflact_webshop_qwen25_3b.py 500 0 > reflact_webshop_qwen_25.txt   # capture stdout
"""
...
RESULTS_LOG = "reflact_webshop_qwen25_3b_results.txt"
PLOT_PREFIX = "reflact_webshop_qwen25_3b_traj_hist"


def llm(prompt, stop=["\n"]):
    # Text-completion endpoint: the ReflAct prompt ending in "Action:" is
    # continued literally by the model, so no system message or action
    # extraction is needed (unlike the chat-model gpt-4.1 variant).
