...
Unlike the ScienceWorld port this is self-contained (the WebShop env/llm helpers
are vendored from reflact_webshop.py), since WebShop has no separate react module.

Examples:
  # Single-env smoke (3 trials of fixed_0):
  python reflexion_webshop_qwen3_8b.py --num-episodes 1 --num-trials 3 --max-steps 15

  # 50-env eval, 3 outer trials each, with JSONL + resume:
  python reflexion_webshop_qwen3_8b.py --num-episodes 50 --num-trials 3 \\
      --results reflexion_webshop_qwen3_8b_results.jsonl --resume
"""
...
# ─── config (matches reflact_webshop.py) ─────────────────────────────────────
...
RESULTS_LOG = "reflexion_webshop_qwen3_8b_results.txt"
PLOT_PREFIX = "reflexion_webshop_qwen3_8b_traj_hist"

# Reflection few-shot examples (WebShop-specific).
...
def generate_reflection(instruction, transcript, memory):
    ...
    text = resp.choices[0].text.replace("</think>", " ").replace("<think>", " ").replace("/no_think", " ").strip()
...
def main():
    print("WebShop Reflexion (multi-trial ReAct + verbal self-reflection, vLLM Qwen3-8B)")
    args = parse_args()
