"""Single-response ReAct agent on ScienceWorld, using the ReflAct paper's prompt.

Prompt: examples/scienceworld_react_prompt.py (Thought / Action / Observation
format, verbatim from Kim et al., EMNLP 2025, Appendix K.2). Each LLM generation
emits one Thought + one Action; we step the env, append the observation, repeat.
...
Examples:
  ...
  python react/react_sciworld.py --task-num 29 --var-num 0 --num-episodes 1 --max-steps 30

  # Full 211-task ReflAct/MPO test set
  python react/react_sciworld.py --mpo-testset --results react/results_react_211.jsonl
"""
...
# Make `scienceworld_react_prompt` importable regardless of CWD (it lives in ../examples).
...
# '/no_think' is a Qwen3 control token; blank it for non-Qwen models. main()
# resets this after parsing --model_name.
...
def parse_thought_action(reply: str):
    """Extract (thought, action) from one LLM response.

    The prompt ends with 'Thought:', so the model continues with the thought
    body followed by an 'Action: <command>' line. Strips Qwen3 <think>/</think>
    markers; ignores any hallucinated Observation or follow-up Thought.
    """
    ...
    for raw in reply.split("\n"):
        ...
        if low.startswith("observation:"):
            break
        if low.startswith("thought:") and thought_lines:
            break
...
def scienceworld_run(env, sw_name, var_idx, simpl_str, max_steps, to_print=True):
    """Play one episode with the paper's ReAct loop.
    ...
    prompt = NOTHINK_PREFIX + SCIENCEWORLD_REACT_PROMPT + task_desc + "\nThought:"
...
def parse_args():
    p = argparse.ArgumentParser(description="Single-response ReAct on ScienceWorld (ReflAct paper prompt, vLLM Qwen3-8B).")
    ...
    p.add_argument("--max-steps", type=int, default=50, help="Max ReAct (LLM) steps per episode (ignored in --mpo-testset; uses per-task caps).")
...
def apply_model_override(args):
    """Reset the module client / model / think-prefix from CLI overrides."""
...
def main():
    ...
    print(f"ScienceWorld ReAct (single-response, {MODEL_NAME}, ReflAct prompt)")
